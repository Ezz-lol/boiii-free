...
DataSources.BoiiiQuickSettings = DataSourceHelpers.ListSetup("BoiiiQuickSettings", function(controller)
	...
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Field of View",
			"Adjust your field of view (65-120).", "QuickSettings_fov",
	...
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "FOV Scale",
			"Multiplier applied to ADS and zoom FOV. 1.0 = default.", "QuickSettings_fovscale",
	...
	local fpsOptions = {}
	local currentFps = 0
	pcall(function() currentFps = Engine.DvarInt(nil, "com_maxfps") end)
	...
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Max FPS",
			"Cap your maximum framerate. 0 = unlimited.", "QuickSettings_maxfps",
			"com_maxfps", fpsOptions, nil, updateDvar))

	-- VSync toggle
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "VSync",
			"Sync framerate to monitor refresh rate.", "QuickSettings_vsync",
			"r_vsync", {
				{
	...
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Show FPS Counter",
			"Display frames per second on screen.", "QuickSettings_drawfps",
			"com_drawFPS_PC", {
				{
					option = "MENU_DISABLED",
					value = 0,
	...
	-- Bot difficulty
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Bot Difficulty",
			"Set AI bot difficulty level.", "QuickSettings_botdifficulty",
			"bot_difficulty", {
				{
					option = "Recruit",
					value = 0,
	...
	if Engine.CurrentSessionMode() == Enum.eModes.MODE_MULTIPLAYER then
		...
		local currentMax = 18
		pcall(function() currentMax = Engine.DvarInt(nil, "com_maxclients") end)
		...
		table.insert(optionsTable,
			CoD.OptionsUtility.CreateDvarSettings(controller, "Max Players",
				"Maximum number of players in custom games.", "QuickSettings_maxclients",
				"com_maxclients", maxClientOptions, nil, updateDvar))
	end

	-- Min players to start (solo play toggle)
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Min Players to Start",
			"Minimum players required to start a match.", "QuickSettings_minplayers",
			"party_minplayers", {
				{
					option = "1 (Solo)",
					value = 1,
	...
	table.insert(optionsTable,
		CoD.OptionsUtility.CreateDvarSettings(controller, "Game Speed",
			"Adjust game speed. 1 = normal.", "QuickSettings_timescale",
			"timescale", {
				{
...
LUI.createMenu.BoiiiQuickSettingsMenu = function(controller)
	...
	self.buttonModel = Engine.CreateModel(Engine.GetModelForController(controller), "BoiiiQuickSettingsMenu.buttonPrompts")
	...
	LUI.OverrideFunction_CallOriginalSecond(self, "close", function(element)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(controller),
			"BoiiiQuickSettingsMenu.buttonPrompts"))
	end)
