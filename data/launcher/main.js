...
var launchOptionCards =
    document.querySelectorAll('#launchDropdown .launch-option-card');
...
var workshopBrowseRefreshBtn =
    document.getElementById('workshopBrowseRefreshBtn');
var workshopBrowseClearBtn = document.getElementById('workshopBrowseClearBtn');
var workshopBrowsePagination =
...
var workshopModalDescription =
    document.getElementById('workshopModalDescription');
var workshopModalViewBtn = document.getElementById('workshopModalViewBtn');
var workshopModalDownloadBtn =
    document.getElementById('workshopModalDownloadBtn');
...
var libraryModalDescription =
    document.getElementById('libraryModalDescription');
...
try {
  ...
  if (ver) {
    ...
    if (vd)
      vd.textContent = 'v' + ver;
    var sv = document.getElementById('settingsVersion');
    if (sv)
      sv.textContent = 'v' + ver;
  }
} catch (e) {
}

try {
  var stored = getExternal() && getExternal().readPlayerName &&
...
try {
  var storedOpts = getExternal() && getExternal().readLaunchOptions &&
  ...
  if (storedOpts) {
    ...
    for (var pi = 0; pi < rawParts.length; pi++) {
      var trimmed =
          (rawParts[pi] || '').toLowerCase().replace(/^\s+|\s+$/g, '');
      if (trimmed)
        parts.push(trimmed);
    }
    for (var i = 0; i < launchOptionCards.length; i++) {
      var optVal = (launchOptionCards[i].getAttribute('data-option') ||
                    '').toLowerCase();
...
function showConfirm(title, body, onConfirm) {
  ...
  document.getElementById('confirmPopupCancel').onclick =
      function() { close(); };
...
if (msgClose)
  msgClose.onclick = hideMessagePopup;
...
function hideProgress() { progressFooter.classList.remove('active'); }
...
function renderWorkshopBrowse() {
  ...
  for (var ai = 0; ai < workshopBrowseItems.length; ai++) {
    ...
    if (item.title && (item.title.indexOf('**ARCHIVE**') !== -1 ||
  ...
  var sortMode =
      workshopFilterSelect ? workshopFilterSelect.value : 'mostrecent';
  if (sortMode === 'trend') {
  ...
  for (var i = start; i < end; i++) {
    ...
    var imageHtml =
        it.imageUrl
            ? '<div class="workshop-item-image"><img src="' +
                  escapeHtml(it.imageUrl) + '" alt=""></div>'
    ...
    if (sr > 0) {
      ...
      for (var s = sr; s < 5; s++)
        starStr += '\u2606';
      var rColor =
          sr >= 4 ? 'rgba(34,197,94,0.8)'
                  : (sr >= 3 ? 'rgba(250,204,21,0.8)' : 'rgba(239,68,68,0.8)');
      ratingHtml = '<span style="font-size:0.72rem;color:' + rColor +
                   ';margin-left:7px;">' + starStr + '</span>';
    }
    ...
    var fStr =
        fSize > 0
            ? (fSize > 1073741824 ? (fSize / 1073741824).toFixed(2) + ' GB'
               ...
               : fSize > 1024     ? (fSize / 1024).toFixed(0) + ' KB'
                                  : fSize + ' B')
            : '';
...
function showWorkshopModal(item) {
  ...
  if (mSr > 0) {
    var mStars = '';
    for (var ms = 0; ms < mSr; ms++)
      mStars += '\u2605';
    for (var ms = mSr; ms < 5; ms++)
      mStars += '\u2606';
    var mColor =
        mSr >= 4 ? 'rgba(34,197,94,0.9)'
                 : (mSr >= 3 ? 'rgba(250,204,21,0.9)' : 'rgba(239,68,68,0.9)');
    modalRatingHtml = '<span style="color:' + mColor + ';">' + mStars + ' (' +
                      mSr + '/5)</span>';
  } else {
    modalRatingHtml =
...
function showLibraryModModal(item) {
  ...
  if (mSr > 0) {
    ...
    modalRatingHtml =
        '<div class="workshop-modal-info-item">' +
        '<div class="workshop-modal-info-label">Rating</div>' +
        '<div class="workshop-modal-info-value"><span style="color:' + mColor +
        ';">' + mStars + ' (' + mSr + '/5)</span></div></div>';
  }
  ...
  var modalTypeHtml = '<div class="workshop-modal-info-item">' +
                      ...
                      '<div class="workshop-modal-info-value">' +
                      escapeHtml((item.type || 'mod') +
                                 (item.source === 'steam' ? ' (Steam)' : '')) +
                      '</div></div>';
  ...
  libraryModalInfo.innerHTML =
      (item.id
           ? '<div class="workshop-modal-info-item">' +
                 '<div class="workshop-modal-info-label">Workshop ID</div>' +
                 '<div class="workshop-modal-info-value" style="font-family:Consolas,monospace;">' +
                 escapeHtml(String(item.id)) + '</div></div>'
           : '') +
  ...
  if (!item.path)
    libraryModalOpenBtn.style.display = 'none';
  else
    libraryModalOpenBtn.style.display = '';
...
if (libraryModalClose)
  libraryModalClose.onclick = hideLibraryModal;
if (libraryOverlay)
  libraryOverlay.onclick = hideLibraryModal;
if (libraryModal)
  libraryModal.onclick = function(e) { e.stopPropagation(); };
...
function doStartWorkshopDownload(id, displayName) {
  try {
    ...
    if (ex && ex.workshopDownload) {
      ...
      if (detEl)
        detEl.textContent = 'Workshop ID: ' + id;
      if (pctEl)
        pctEl.textContent = '';
  ...
  if (window._workshopPollInterval) {
    showConfirm(
        ...
        'A download is already in progress. Stop it and download "' +
            (displayName || id) + '" instead?',
        function() {
          stopWorkshopDownloadCompletely();
          setTimeout(function() { doStartWorkshopDownload(id, displayName); },
                     300);
        });
    return;
  }
...
function pollWorkshopStatus() {
  try {
    ...
    if (status.message && status.message.length > 0) {
      ...
      if (detEl)
        detEl.textContent = status.details || '';
      ...
      if (status.progress !== undefined && status.progress > 0) {
        ...
        if (!window._wsIndeterminate) {
          ...
          if (pf3)
            pf3.classList.add('indeterminate');
        }
        if (pctEl)
          pctEl.textContent = '';
...
if (workshopProgressCancel)
  workshopProgressCancel.onclick =
      function() { stopWorkshopDownloadCompletely(); };
  ...
  workshopProgressPause.onclick = function() {
    try {
      ...
      if (window._workshopIsPaused) {
        if (ex.workshopResumeDownload)
          ex.workshopResumeDownload();
        window._workshopIsPaused = false;
        ...
        if (ex.workshopPauseDownload)
          ex.workshopPauseDownload();
        window._workshopIsPaused = true;
        workshopProgressPause.innerHTML = '&#9654;';
...
if (progressPauseBtn) {
  progressPauseBtn.onclick = function() {
    if (workshopProgressPause)
      workshopProgressPause.click();
  };
}

document.getElementById('progressCancel').onclick =
    function() { stopWorkshopDownloadCompletely(); };
...
function startRemoveProgressPoll() {
  ...
  if (_removeProgressPollInterval)
    ...
    if (ex.workshopListAsync) {
      ...
      _modsListPollInterval = setInterval(function() {
        ...
        try {
          ...
          if (!ex2)
            return;
          var isLoading = ex2.workshopListIsLoading &&
...
function renderModsPage() {
  ...
  for (var i = start; i < end; i++) {
    ...
    if (sr > 0) {
      ...
      for (var s = sr; s < 5; s++)
        sStr2 += '\u2606';
      var rColor2 = sr >= 4 ? 'rgba(34,197,94,0.75)'
                            : (sr >= 3 ? 'rgba(250,204,21,0.75)'
                                       : 'rgba(239,68,68,0.75)');
    ...
    card.onclick = (function(it) {
      return function(e) {
        if (e.target.closest && e.target.closest('.mod-card-actions'))
          return;
  ...
  if (modsPagination) {
    if (totalPages > 1) {
      ...
      if (modsPrevPageBtn)
        modsPrevPageBtn.disabled = modsPage <= 1;
      if (modsNextPageBtn)
        modsNextPageBtn.disabled = modsPage >= totalPages;
    } else {
      modsPagination.style.display = 'none';
    }
  }
...
document.getElementById('workshopRefreshBtn').onclick =
    function() { refreshModsGrid(); };
    ...
    for (var ci = 0; ci < items.length; ci++) {
      ...
      if (cSz2 > 0) {
        cSzStr = cSz2 > 1073741824 ? (cSz2 / 1073741824).toFixed(2) + ' GB'
                 ...
                 : cSz2 > 1024     ? (cSz2 / 1024).toFixed(0) + ' KB'
                                   : cSz2 + ' B';
      }
      lh += '<li><span class="item-name">' + cName + '</span>' +
      ...
      try {
        ...
        if (!isLoading) {
          ...
          for (var ui = 0; ui < items2.length; ui++) {
            if (items2[ui].needsUpdate && items2[ui].id &&
...
document.getElementById('manageInstallBtn').onclick = function() {
  if (manageInstallPopup)
    manageInstallPopup.classList.add('active');
  loadComponents();
};

function hideManagePopup() {
  if (manageInstallPopup)
    manageInstallPopup.classList.remove('active');
}
...
manageInstallPopup.onclick = function(e) {
  if (e.target === manageInstallPopup)
    hideManagePopup();
};
...
function loadComponents() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var sizeText = installed ? info.sizeHuman + ' (' + info.count + ' files)'
                             : 'Not installed';
    ...
    div.innerHTML =
        ...
        '<input type="checkbox" id="comp-' + it.id + '" data-comp="' + it.id +
        '"' + (installed ? '' : ' disabled') + '>' +
        '<span class="checkbox-visual"></span></div>' +
        '<div class="component-info"><div class="component-name">' + it.name +
        '</div>' +
...
document.getElementById('popupApply').onclick = function() {
  ...
  for (var key in componentSelection) {
    if (componentSelection[key] && modeFilesInfo[key] &&
              ...
              function() {
                try {
                  ...
                  if (ex && ex.removeModeFiles) {
                    ...
                    var r = typeof result === 'string' ? JSON.parse(result)
                                                       : result;
                    var msg = 'Removed ' + r.removedCount + ' files (' +
                              r.removedSize + ')';
                    if (r.failedCount > 0)
...
function showVerifyResults(changedFiles, message) {
  ...
  if (!changedFiles || changedFiles.length === 0) {
    ...
    for (var i = 0; i < changedFiles.length; i++) {
      ...
      var color = isWarning ? 'color:rgba(234,179,8,0.88)'
                            : 'color:rgba(239,68,68,0.88)';
      html += '<div class="verify-file-item" style="' + color + '">' +
  ...
  try {
    ...
    verifyPollInterval = setInterval(function() {
      try {
        ...
        if (!verifySeenRunning)
          return;
        var rawPct = Math.min(st.progress || 0, 100);
        if (rawPct > verifyMaxPct)
          verifyMaxPct = rawPct;
...
setupPathBtn('settingsChangePathBtn',
             [ 'currentPathDisplay', 'settingsPathDisplay' ]);
...
var ASSET_POOL_DEFAULTS = {
  ...
  ap_weaponcamo : 256,
  ap_sharedweaponsounds : 128,
  ap_fx : 1024,
  ap_tagfx : 256,
  ap_newlensflaredef : 128,
  ap_aitype : 256,
  ap_character : 256,
  ap_rawfile : 2048,
  ap_stringtable : 512,
...
var allSettingToggles =
    document.querySelectorAll('.settings-toggle[data-setting]');
  ...
  input.onchange = function() {
    ...
    try {
      ...
      if (ex && ex.setGameSetting)
        ex.setGameSetting(configKey, val);
    } catch (e) {
    }
  };
}
  ...
  (function(input) {
    input.onchange = function() {
      try {
        ...
        var ex = getExternal();
        if (ex && ex.setGameSetting)
          ex.setGameSetting(poolKey, String(val));
      } catch (e) {
      }
    };
  })(allPoolInputs[pi]);
...
function loadGameSettings() {
  try {
    ...
    if (s.MaxFrameLatency !== undefined) {
      ...
      if (el)
        el.value = s.MaxFrameLatency;
    }
    if (s.VideoMemory !== undefined) {
      var fv = (s.VideoMemory === '1' || s.VideoMemory === 1) &&
    ...
    if (s.friendsOnly !== undefined) {
      var fo = s.friendsOnly === '1' || s.friendsOnly === 1 ||
               s.friendsOnly === true;
      setSetting('friendsOnly', fo);
      var ft2 = document.getElementById('settingsFriendsOnly');
      if (ft2) {
        if (fo)
          ft2.classList.add('active');
        else
          ft2.classList.remove('active');
      }
    }

    if (s.reduceStutter !== undefined) {
      setSetting('reduceStutter',
    ...
    if (s.assetLimits !== undefined) {
      var al = s.assetLimits === '1' || s.assetLimits === 1 ||
               s.assetLimits === true;
    ...
    showConfirm(
        ...
        function() {
          try {
            ...
            if (ex) {
              if (ex.toggleReduceStutter)
                ex.toggleReduceStutter('0');
              if (ex.toggleSkipIntro)
                ex.toggleSkipIntro('0');
              if (ex.toggleSkipAllIntros)
                ex.toggleSkipAllIntros('0');
              if (ex.setGameSetting) {
          ...
          applySettingsToUI();
          if (networkPwInput)
            networkPwInput.value = '';
          if (netPasswordInput)
            netPasswordInput.value = '';
          loadGameSettings();
          showMessage(
              'Settings Reset',
...
if (assetPoolsResetBtn) {
  assetPoolsResetBtn.onclick = function() {
    showConfirm(
        'Reset Asset Pools',
        'Reset all asset pool sizes to their default values?', function() {
          try {
            var ex = getExternal();
...
if (versionOptions) {
  ...
  if (defaultOpt) {
    defaultOpt.onclick =
        function() { selectVersion('latest', 'Latest (Auto-update)'); };
  }
}
