...
Check-Dependencies -Deps @("git", "clang-format", "stylua", "shfmt", "yarn", "dos2unix")
...
function Should-Format {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Path)) {
        ...
        if (-not ($pathStr -match "^deps/") -or ($pathStr -match "^deps/premake/")) {
            return $true
        }
    }
    return $false
...
foreach ($file in $files) {
    if (Should-Format -Path $file) {
        $absPath = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($RepoDir, $file))
...
$formatScriptBlock = {
    ...
    try {
        switch -Regex ($ext) {
            "^lua$" {
                Invoke-CommandOrThrow { stylua $path }
                Invoke-CommandOrThrow { dos2unix --quiet $path }
                break
            }
            "^(md|markdown|html|htm|js|jsx|ts|tsx|mjs|cjs|mts|cts|graphql|gql|json|json5|webmanifest|yaml|yml|vue|svelte|handlebars|hbs|toml|ps1)$" {
                ...
                Invoke-CommandOrThrow { cmd /c "yarn run prettier --config `"$prettierConfig`" --write `"$path`"" }
                Invoke-CommandOrThrow { dos2unix --quiet $path }
                break
            }
            "^sh$" {
                Invoke-CommandOrThrow { shfmt --write $path }
                break
            }
            "^(c|cc|cpp|cxx|hpp|h|hxx|js|json|proto|java|cs|objc|sv|svh|v|svp|svl|vlg|vt|vh|verilog|vo|vqm|vcd)$" {
                Invoke-CommandOrThrow { clang-format -i --style=file $path }
                Invoke-CommandOrThrow { dos2unix --quiet $path }
                break
            }
        }
...
try {
    ...
    foreach ($file in $filesToFormat) {
        $ps = [powershell]::Create().AddScript($formatScriptBlock).AddArgument($file).AddArgument($RepoDir)
