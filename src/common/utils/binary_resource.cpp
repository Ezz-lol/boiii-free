...
std::string write_existing_temp_file(const std::string &file,
  ...
  if (current_data == data || io::write_file(file_path, data) ||
      !fatal_if_overwrite_fails) {
    return file_path;
  }

  throw std::runtime_error("Temporary file was already written, but differs. "
                           "It can't be overwritten as it's still in use: " +
                           file_path);
}
} // namespace
...
std::string
binary_resource::get_extracted_file(const bool fatal_if_overwrite_fails) {
  if (this->path_.empty()) {
...
const std::string &binary_resource::get_data() const { return this->resource_; }
} // namespace utils
