...
image load_image(const std::string &data) {
  ...
  res.data.assign(reinterpret_cast<const char *>(buffer),
                  res.width * res.height * channels);
...
object create_bitmap(const image &img) {
  ...
  return CreateBitmap(static_cast<int>(img.width), static_cast<int>(img.height),
                      4, 8, copy.data());
}
} // namespace utils::image
