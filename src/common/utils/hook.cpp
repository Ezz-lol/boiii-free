...
uint8_t *allocate_somewhere_near(const void *base_address,
                                 const size_t granularity, const size_t size) {
...
public:
  ...
  memory(const void *ptr) : memory() {
    ...
    this->buffer_ =
        allocate_somewhere_near(ptr, allocation_granularity, this->length_);
    if (!this->buffer_) {
  ...
  memory(memory &&obj) noexcept : memory() { this->operator=(std::move(obj)); }
  ...
  void *get_ptr() const { return this->buffer_ + this->offset_; }
...
detour::detour(void *place, void *target) : detour() {
...
void detour::move() { this->moved_data_ = move_hook(this->place_); }
...
std::optional<std::pair<void *, void *>> iat(const nt::library &library,
                                             const std::string &target_library,
...
bool is_relatively_far(const void *pointer, const void *data,
                       const int offset) {
  return is_relatively_far(reinterpret_cast<size_t>(pointer),
                           reinterpret_cast<size_t>(data), offset);
}
...
void jump(void *pointer, void *data, const bool use_far, const bool use_safe) {
  ...
  static const unsigned char jump_data_safe[] = {0xFF, 0x25, 0x00,
                                                 0x00, 0x00, 0x00};
  ...
  if (use_far) {
    ...
    *reinterpret_cast<int32_t *>(&copy_data[1]) =
        static_cast<int32_t>(size_t(data) - (size_t(pointer) + 5));
...
void inject(void *pointer, const void *data) {
  return inject(reinterpret_cast<size_t>(pointer),
