...
public:
  ...
  void add_entropy(const void *data, const size_t length) const {
    this->descriptor_.add_entropy(static_cast<const uint8_t *>(data),
                                  ul(length), this->state_.get());
  }

  void read(void *data, const size_t length) const {
    this->descriptor_.read(static_cast<unsigned char *>(data), ul(length),
                           this->get_state());
  }
...
std::string ecc::key::get_openssl() const {
  ...
  if (ecc_export_openssl(buffer, &length, PK_PUBLIC, &this->key_storage_) ==
      CRYPT_OK) {
    return {cs(buffer), length};
  }
...
void ecc::key::set_openssl(const std::string &key) {
  ...
  if (ecc_import_openssl(cs(key.data()), ul(key.size()), &this->key_storage_) !=
      CRYPT_OK) {
    ZeroMemory(&this->key_storage_, sizeof(this->key_storage_));
  }
}
...
std::string rsa::encrypt(const std::string &data, const std::string &hash,
  ...
  const auto _ = finally([&]() { rsa_free(&new_key); });
...
void random::get_data(void *data, const size_t size) { prng_.read(data, size); }
} // namespace utils::cryptography