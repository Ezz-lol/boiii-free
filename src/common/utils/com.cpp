    ...
    x() {
      if (FAILED(CoInitialize(nullptr))) {
        throw std::runtime_error(
            "Failed to initialize the component object model");
      }
    }
...
bool select_folder(std::string &out_folder, const std::string &title,
  ...
  if (!selected_folder.empty()) {
    ...
    if (SUCCEEDED(SHCreateItemFromParsingName(wide_sel.c_str(), nullptr,
                                              IID_PPV_ARGS(&psi_default))) &&
        psi_default) {
  ...
  bool ok =
      SUCCEEDED(psi->GetDisplayName(SIGDN_FILESYSPATH, &path_buf)) && path_buf;
  if (ok) {
