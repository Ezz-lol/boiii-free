...
namespace {
void set_bits(uint64_t &value, const uint32_t bit_index, const uint32_t bits,
...
public:
  ...
  ~debug_context() { SetThreadContext(this->handle_, &this->context_); }
...
uint32_t activate(const uint64_t address, uint32_t length, const condition cond,
                  CONTEXT &context) {
...
uint32_t activate(void *address, const uint32_t length, const condition cond,
                  const uint32_t thread_id) {
  return activate(reinterpret_cast<uint64_t>(address), length, cond, thread_id);
}

uint32_t activate(const uint64_t address, const uint32_t length,
                  const condition cond, const uint32_t thread_id) {
  debug_context context(thread_id);
...
void deactivate_all(CONTEXT &context) { context.Dr7 = 0; }
