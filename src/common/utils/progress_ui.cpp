...
namespace utils
{
	namespace
	{
		...
		{
			return {rc.right - PADDING - BTN_W,
			        rc.bottom - PADDING - BTN_H + 4,
			        rc.right - PADDING,
			        rc.bottom - PADDING + 4};
		}
	...
	void progress_ui::set_progress(const size_t current, const size_t max) const
	{
		...
		if (hwnd_) PostMessage(hwnd_, WM_UPDATE_UI, 0, 0);
	}
	...
	void progress_ui::set_line(const int line, const std::string& text) const
	{
		...
		{
			std::lock_guard<std::recursive_mutex> lock(state_mutex_);
			if (line == 1) state_.line1 = text;
			else if (line == 2) state_.line2 = text;
		}
	...
	{
		if (hwnd_ || headless_) return;

		thread_ready_ = false;

		ui_thread_ = std::thread([this]()
		{
			const auto class_name = "BOIII_ProgressUI_" + std::to_string(reinterpret_cast<uintptr_t>(this));
			...
			if (hwnd_)
			{
				...
				{
					const int x = mi.rcWork.left + (mi.rcWork.right - mi.rcWork.left - WINDOW_W) / 2;
					const int y = mi.rcWork.top + (mi.rcWork.bottom - mi.rcWork.top - WINDOW_H) / 2;
	...
	LRESULT CALLBACK progress_ui::wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
	{
		...
		{
			wd = reinterpret_cast<window_data*>(GetWindowLongPtrA(hwnd, GWLP_USERDATA));
		}
		...
		case WM_MOUSEMOVE:
		{
			...
			if (wd->dragging)
			{
				...
				MoveWindow(hwnd,
				           wr.left + (screen_pt.x - wd->drag_start.x),
				           wr.top + (screen_pt.y - wd->drag_start.y),
		...
		case WM_LBUTTONDOWN:
		{
			...
			{
				if (wd->self) wd->self->cancelled_ = true;
				return 0;
			}
	...
	void progress_ui::paint(HDC hdc, const RECT& rc) const
	{
		...
		HFONT title_font = CreateFontA(17, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
		...
		auto close_rc = get_close_btn_rect(rc);
		auto* wd = reinterpret_cast<window_data*>(GetWindowLongPtrA(hwnd_, GWLP_USERDATA));
		...
		HFONT close_font = CreateFontA(20, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
		                               DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, "Segoe UI");
		...
		HFONT text_font = CreateFontA(16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
		                              DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, "Segoe UI");
		...
		if (marquee)
		{
			...
			if (fill_start < bar_left) fill_start = bar_left;
			if (fill_end > bar_right) fill_end = bar_right;
		...
		{
			double pct = static_cast<double>(cur) / static_cast<double>(max_val) * 100.0;
		...
		auto cancel_rc = get_cancel_btn_rect(rc);
		COLORREF cancel_bg = (wd && wd->cancel_hover) ? COL_CANCEL_HOVER : COL_CANCEL_BG;
		...
		RoundRect(hdc, cancel_rc.left, cancel_rc.top, cancel_rc.right, cancel_rc.bottom, 5, 5);
		SelectObject(hdc, old_pen);
		SelectObject(hdc, old_brush);
		DeleteObject(cancel_pen);

		HFONT btn_font = CreateFontA(15, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE,
		                             DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, "Segoe UI");
		...
		DrawTextA(hdc, btn_text.c_str(), -1, &cancel_rc, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		SelectObject(hdc, old_font);
		DeleteObject(btn_font);
	}

	void progress_ui::show_error(const std::string& title, const std::string& message)
	{
