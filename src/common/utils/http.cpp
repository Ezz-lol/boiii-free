...
size_t write_callback_stream(void *contents, const size_t size,
  ...
  try {
    if (write_helper->callback && *write_helper->callback) {
      (*write_helper->callback)(static_cast<const char *>(contents),
                                total_size);
    }
  } catch (...) {
...
std::optional<std::string> get_data(const std::string &url,
  ...
  header_list =
      curl_slist_append(header_list, "Accept: application/octet-stream");
