...
bool add_file(zipFile &zip_file, const std::string &filename,
  ...
  if (ZIP_OK != zipOpenNewFileInZip64(zip_file, filename.data(), nullptr,
                                      nullptr, 0, nullptr, 0, nullptr,
                                      Z_DEFLATED, Z_BEST_COMPRESSION, zip_64)) {
    return false;
  }

  const auto _ = finally([&zip_file]() { zipCloseFileInZip(zip_file); });

  size_t offset = 0;
  while (offset < data.size()) {
    const auto chunk_size =
        std::min(data.size() - offset, static_cast<size_t>(UINT_MAX));
    if (ZIP_OK != zipWriteInFileInZip(zip_file, data.data() + offset,
...
read_zip_file_entry(unzFile &zip_file) {
  ...
  if (unzGetCurrentFileInfo(zip_file, &file_info, filename, sizeof(filename),
...
class memory_file {
public:
  memory_file(const std::string &data) : data_(data) {
  ...
  const char *get_name() const { return "blub"; }

  zlib_filefunc64_def *get_func_def() { return &this->func_def_; }
...
private:
  ...
  static voidpf open_file_static(const voidpf opaque, const void *filename,
                                 const int mode) {
    return static_cast<memory_file *>(opaque)->open_file(filename, mode);
  }

  static long seek_file_static(const voidpf opaque, const voidpf stream,
                               const ZPOS64_T offset, const int origin) {
    return static_cast<memory_file *>(opaque)->seek_file(stream, offset,
                                                         origin);
  }
  ...
  static uLong read_file_static(const voidpf opaque, const voidpf stream,
                                void *buf, const uLong size) {
  ...
  static int close_file_static(voidpf, voidpf) { return 0; }

  static int testerror_file_static(voidpf, voidpf) { return 0; }
};
} // namespace
