...
PIMAGE_TLS_DIRECTORY allocate_tls_index() {
  static std::atomic_bool already_allocated = false;
  ...
  const uint32_t tls_dir_entry = tls_dll.get_optional_header()
  ...
  return reinterpret_cast<PIMAGE_TLS_DIRECTORY>(tls_dll.get_ptr() +
                                                tls_dir_entry);
}
} // namespace tls