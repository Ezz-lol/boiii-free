...
#include "../steam.hpp"
#include <component/dedicated/command.hpp>

namespace steam {
bool game_server::InitGameServer(unsigned int unGameIP,
                                 unsigned short unGamePort,
                                 unsigned short usQueryPort,
                                 unsigned int unServerFlags,
                                 unsigned int nAppID, const char *pchVersion) {
  return true;
}
...
bool game_server::BLoggedOn() { return true; }

bool game_server::BSecure() { return false; }

steam_id game_server::GetSteamID() { return SteamUser()->GetSteamID(); }

bool game_server::WasRestartRequested() { return false; }
...
int game_server::SendUserConnectAndAuthenticate(unsigned int unIPClient,
...
bool game_server::BUpdateUserData(steam_id steamIDUser,
                                  const char *pchPlayerName,
                                  unsigned int uScore) {
  return false;
}

int game_server::GetAuthSessionTicket(void *pTicket, int cbMaxTicket,
                                      unsigned int *pcbTicket) {
  return 0;
}

int game_server::BeginAuthSession(const void *pAuthTicket, int cbAuthTicket,
                                  steam_id steamID) {
  return 0;
}
...
unsigned long long game_server::GetServerReputation() { return 0; }

unsigned int game_server::GetPublicIP() { return 0; }

bool game_server::HandleIncomingPacket(const void *pData, int cbData,
                                       unsigned int srcIP,
                                       unsigned short srcPort) {
  return false;
}

int game_server::GetNextOutgoingPacket(void *pOut, int cbMaxOut,
...
unsigned long long game_server::AssociateWithClan(steam_id clanID) { return 0; }

unsigned long long
game_server::ComputeNewPlayerCompatibility(steam_id steamID) {
  return 0;
}
} // namespace steam