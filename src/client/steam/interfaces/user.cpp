...
unsigned int user::GetAuthSessionTicket(void *pTicket, int cbMaxTicket,
  ...
  *pcbTicket = 1;
  ...
  auto *response = static_cast<get_auth_session_ticket_response *>(
...
unsigned long long user::RequestEncryptedAppTicket(void *pUserData,
                                                   int cbUserData) {
  ...
  const auto result = callbacks::register_call();
  const auto retvals = static_cast<encrypted_app_ticket_response *>(
