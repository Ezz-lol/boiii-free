...
namespace steam {
uint64_t
ugc::CreateQueryUserUGCRequest(uint_least32_t unAccountID, uint32_t eListType,
                               uint32_t eMatchingUGCType, uint32_t eSortOrder,
                               uint32_t nCreatorAppID, uint32_t nConsumerAppID,
                               uint32_t unPage) {
  return 0;
}

uint64_t ugc::CreateQueryAllUGCRequest(
    uint32_t eQueryType, uint32_t eMatchingeMatchingUGCTypeFileType,
    uint32_t nCreatorAppID, uint32_t nConsumerAppID, uint32_t unPage) {
  return 0;
}
...
bool ugc::GetQueryUGCPreviewURL(uint64_t handle, uint32_t index, char *pchURL,
                                uint32_t cchURLSize) {
  return false;
}

bool ugc::GetQueryUGCMetadata(uint64_t handle, uint32_t index,
                              char *pchMetadata, uint32_t cchMetadatasize) {
  return false;
}

bool ugc::GetQueryUGCChildren(uint64_t handle, uint32_t index,
                              uint64_t *pvecPublishedFileID,
                              uint32_t cMaxEntries) {
  return false;
}

bool ugc::GetQueryUGCStatistic(uint64_t handle, uint32_t index,
                               uint32_t eStatType, uint32_t *pStatValue) {
  return false;
}

uint32_t ugc::GetQueryUGCNumAdditionalPreviews(uint64_t handle,
                                               uint32_t index) {
  return 0;
}

bool ugc::GetQueryUGCAdditionalPreview(
    uint64_t handle, uint32_t index, uint32_t previewIndex,
...
bool ugc::GetQueryUGCKeyValueTag(uint64_t handle, uint32_t index,
                                 uint32_t keyValueTagIndex, char *pchKey,
                                 uint32_t cchKeySize, char *pchValue,
                                 uint32_t cchValueSize) {
  return false;
}

bool ugc::ReleaseQueryUGCRequest(uint64_t handle) { return false; }
...
bool ugc::SetReturnLongDescription(uint64_t handle,
...
bool ugc::SetReturnAdditionalPreviews(uint64_t handle,
...
bool ugc::SetCloudFileNameFilter(uint64_t handle,
                                 const char *pMatchCloudFileName) {
  return false;
}

bool ugc::SetMatchAnyTag(uint64_t handle, bool bMatchAnyTag) { return false; }
...
bool ugc::AddRequiredKeyValueTag(uint64_t handle, const char *pKey,
                                 const char *pValue) {
  return false;
}

uint64_t ugc::RequestUGCDetails(uint64_t nPublishedFileID,
...
uint64_t ugc::StartItemUpdate(uint32_t nConsumerAppId,
                              uint64_t nPublishedFileID) {
  return 0;
}

bool ugc::SetItemTitle(uint64_t handle, const char *pchTitle) { return false; }

bool ugc::SetItemDescription(uint64_t handle, const char *pchDescription) {
  return false;
}

bool ugc::SetItemUpdateLanguage(uint64_t handle,
                                const char *pchUpdateLanguage) {
  return false;
}
...
bool ugc::AddItemKeyValueTag(uint64_t handle, const char *pchKey,
                             const char *pchValue) {
  return false;
}

bool ugc::AddItemPreviewFile(uint64_t handle, const char *pszPreviewFile,
                             uint32_t type) {
  return false;
}
...
bool ugc::UpdateItemPreviewFile(uint64_t handle, uint32_t index,
                                const char *pszPreviewFile) {
  return false;
}

bool ugc::UpdateItemPreviewVideo(uint64_t handle, uint32_t index,
                                 const char *pszVideoID) {
  return false;
}

bool ugc::RemoveItemPreview(uint64_t handle, uint32_t index) { return false; }

uint64_t ugc::SubmitItemUpdate(uint64_t handle, const char *pchChangeNote) {
  return 0;
}

uint32_t ugc::GetItemUpdateProgress(uint64_t handle,
...
uint64_t ugc::GetUserItemVote(uint64_t nPublishedFileID) { return 0; }
...
uint32_t ugc::GetNumSubscribedItems() {
  ...
  uint32_t count = 0;
  steam_proxy::access_subscribed_items(
      [&](const steam_proxy::subscribed_item_map &items) {
...
uint32_t ugc::GetSubscribedItems(uint64_t *pvecPublishedFileID,
                                 uint32_t cMaxEntries) {
  uint32_t count = 0;
  steam_proxy::access_subscribed_items(
      [&](const steam_proxy::subscribed_item_map &items) {
...
bool ugc::GetItemInstallInfo(uint64_t nPublishedFileID, uint64_t *punSizeOnDisk,
      ...
      [&](const steam_proxy::subscribed_item_map &items) {
        ...
        if (entry != items.end()) {
          ...
          memcpy(pchFolder, item.path.data(),
                 std::min(item.path.size() + 1,
                          static_cast<size_t>(cchFolderSize)));
...
bool ugc::GetItemDownloadInfo(uint64_t nPublishedFileID,
...
bool ugc::BInitWorkshopForGameServer(uint64_t unWorkshopDepotID,
...
uint64_t ugc::StopPlaytimeTrackingForAllItems() { return 0; }
} // namespace steam
