...
std::vector<file_info> get_file_infos() {
  const auto json =
      utils::http::get_data(get_update_file() + get_cache_buster());
  if (!json) {
    return {};
  }
...
const file_info *
find_host_file_info(const std::vector<file_info> &outdated_files) {
...
file_updater::file_updater(progress_listener &listener,
    ...
    : listener_(listener), base_(std::move(base)),
...
void file_updater::create_config_file_if_not_exists() const {
  ...
  const auto config_path =
      std::filesystem::path(buffer) / "Activision" / "CoD" / "config.ini";
      ...
      if (hash != host_file->hash) {
        if (!utils::flags::has_flag("update")) {
          OutputDebugStringA("WARNING: Host binary is outdated but not "
                             "updating in debug build\n");
          OutputDebugStringA(
              "Run with -update flag to enable exe updates in debug mode\n");
        }
      }
    }
  }
...
void file_updater::update_file(const file_info &file) const {
  ...
  if (is_exe) {
    OutputDebugStringA(
        ("Writing exe to temp file first: " + out_file.string() + "\n")
            .c_str());
    ...
    if (!utils::io::write_file_executable(temp_path, *data)) {
      throw std::runtime_error("Failed to write temp exe: " + file.name +
                               " to " + temp_path.string());
    }
    ...
    if (!utils::io::read_file(temp_path, &verify_data)) {
      throw std::runtime_error(
          "Failed to read back temp exe for verification: " +
          temp_path.string());
    }
    ...
    if (verify_data.size() != data->size()) {
      throw std::runtime_error("Temp exe size mismatch after write: expected " +
                               ...
                               std::to_string(verify_data.size()));
    }
    ...
    OutputDebugStringA(
        ("Moving temp exe to final location: " + out_file.string() + "\n")
            .c_str());
    ...
    if (!MoveFileExW(temp_path.wstring().c_str(), out_file.wstring().c_str(),
      ...
      throw std::runtime_error("Failed to replace exe: " + file.name +
                               " (MoveFileEx failed with error " +
                               std::to_string(error) + ")");
    }

    OutputDebugStringA(
        ("Successfully replaced executable: " + file.name + "\n").c_str());
  } else {
    OutputDebugStringA(("Writing to: " + out_file.string() + "\n").c_str());

    if (!utils::io::write_file(out_file, *data, false)) {
      throw std::runtime_error("Failed to write: " + file.name + " to " +
                               out_file.string() +
...
file_updater::get_outdated_files(const std::vector<file_info> &files) const {
...
void file_updater::update_host_binary(
  ...
  try {
    ...
    for (auto i = 0; i < max_retries; ++i) {
      try {
        OutputDebugStringA(("Attempting exe update, attempt " +
                            std::to_string(i + 1) + "/" +
...
void file_updater::update_files(
  ...
  const auto thread_count =
      get_optimal_concurrent_download_count(outdated_files.size());
...
bool file_updater::is_outdated_file(const file_info &file) const {
  ...
  if (!utils::io::read_file(drive_name, &data)) {
    OutputDebugStringA(
        ("File not found, marking as outdated: " + file.name + "\n").c_str());
    return true;
  }

  if (data.size() != file.size) {
    OutputDebugStringA(("Size mismatch for " + file.name +
                        ": local=" + std::to_string(data.size()) +
...
std::filesystem::path
file_updater::get_drive_filename(const file_info &file) const {
  if (file.name == UPDATE_HOST_BINARY) {
...
void file_updater::move_current_process_file() const {
  ...
  if (attrs != INVALID_FILE_ATTRIBUTES && (attrs & FILE_ATTRIBUTE_READONLY)) {
    ...
    SetFileAttributesW(this->process_file_.wstring().c_str(),
  ...
  throw std::runtime_error(
      "Failed to move current process file after 5 attempts from " +
      this->process_file_.string() + " to " +
      this->dead_process_file_.string());
}

void file_updater::restore_current_process_file() const {
  OutputDebugStringA(("Restoring exe from " +
                      this->dead_process_file_.string() + " to " +
  ...
  if (attrs != INVALID_FILE_ATTRIBUTES && (attrs & FILE_ATTRIBUTE_READONLY)) {
    ...
    SetFileAttributesW(this->dead_process_file_.wstring().c_str(),
                       attrs & ~FILE_ATTRIBUTE_READONLY);
  }
...
void file_updater::delete_old_process_file() const {
  ...
  for (auto i = 0; i < 4; ++i) {
    ...
    OutputDebugStringA(
        ("Failed to delete old process file, retrying (attempt " +
         std::to_string(i + 1) + "/4)...\n")
            .c_str());
  ...
  if (utils::io::file_exists(this->dead_process_file_)) {
    OutputDebugStringA(
        "Warning: Could not delete old process file after 4 attempts\n");
  }
}

void file_updater::cleanup_directories(
...
void file_updater::cleanup_data_directory(
    const std::vector<file_info> &files) const {
