		...
		ast_ptr parse_primary(parser_state& s)
		{
			...
			if (s.check(token_type::t_lbracket))
			{
				if (s.peek(1).type == token_type::t_lbracket)
				{
					...
					call->children.push_back(make_node(node_type::n_undefined, "undefined", t.line, t.column));
			...
			{
				auto& tok = s.advance();
				return make_node(node_type::n_float_number, tok.value, tok.line, tok.column);
			}
			...
			{
				s.advance();
				auto& name = s.expect(token_type::t_identifier, "Expected function name after '::'");
			...
			if (s.check(token_type::t_identifier))
			{
				...
				if (s.check(token_type::t_double_colon))
				{
					...
					if (s.check(token_type::t_identifier))
					{
						...
						{
							s.advance();
							auto call = make_node(node_type::n_call, func_name, tok.line, tok.column);
							call->children.push_back(make_node(node_type::n_identifier, name, tok.line, tok.column));
						...
						auto ref = make_node(node_type::n_func_ref, func_name, tok.line, tok.column);
						ref->children.push_back(make_node(node_type::n_identifier, name, tok.line, tok.column));
						return ref;
					}
				}
				...
				if (s.check(token_type::t_lparen))
				{
					...
					call->children.push_back(make_node(node_type::n_identifier, "", tok.line, tok.column));
					auto args = make_node(node_type::n_block, "args", tok.line, tok.column);
					if (!s.check(token_type::t_rparen))
					{
						args->children.push_back(parse_expression(s));
			...
			if (s.check(token_type::t_thread))
			{
				...
				auto thread_node = make_node(node_type::n_thread_call, "thread", t.line, t.column);
			...
			while (true)
			{
				if (s.check(token_type::t_dot))
				{
					...
					if (s.check(token_type::t_identifier))
					{
						...
						{
							auto size_node = make_node(node_type::n_size, "size", field_tok.line, field_tok.column);
							size_node->children.push_back(std::move(expr));
							expr = std::move(size_node);
						}
						else
						{
							auto field = make_node(node_type::n_field_access, field_tok.value, field_tok.line, field_tok.column);
				...
				if (s.check(token_type::t_lbracket) && s.peek(1).type != token_type::t_lbracket)
				{
					s.advance();
					auto key = parse_expression(s);
					s.expect(token_type::t_rbracket, "Expected ']'");
					auto arr = make_node(node_type::n_array_access, "", expr->line, expr->column);
				...
				auto is_method_start = [&]() -> bool {
					return s.check(token_type::t_identifier) || s.check(token_type::t_thread)
						|| s.check(token_type::t_waittill) || s.check(token_type::t_waittillmatch)
						|| s.check(token_type::t_notify_kw) || s.check(token_type::t_endon_kw);
				};
				...
				if (is_method_start())
				{
					...
					auto is_callable_token = [&]() -> bool {
						return s.check(token_type::t_identifier) || s.check(token_type::t_waittill)
							|| s.check(token_type::t_waittillmatch) || s.check(token_type::t_notify_kw)
					...
					if (is_callable_token())
					{
						...
						if (next.type == token_type::t_lparen)
						{
							...
							if (method_name == "waittill") nt = node_type::n_waittill;
							else if (method_name == "notify") nt = node_type::n_notify;
							else if (method_name == "endon") nt = node_type::n_endon;

							s.advance();
							auto call = make_node(nt, method_name, method_tok.line, method_tok.column);
									call->children.push_back(std::move(expr));
							auto args = make_node(node_type::n_block, "args", method_tok.line, method_tok.column);
							if (!s.check(token_type::t_rparen))
							{
								args->children.push_back(parse_expression(s));
								while (s.match(token_type::t_comma))
...
auto ns_node = make_node(node_type::n_identifier, ns, ns_tok.line, ns_tok.column);
					...
					if (s.check(token_type::t_lbracket) && s.peek(1).type == token_type::t_lbracket)
					{
						...
						auto call = make_node(node_type::n_call_ptr, "", expr->line, expr->column);
						call->children.push_back(std::move(expr));
						call->children.push_back(std::move(ptr_expr));
				...
				{
					s.advance();
					auto node = make_node(node_type::n_inc_dec, "post++", expr->line, expr->column);
		...
		int get_precedence(token_type t)
		{
			switch (t)
			{
			case token_type::t_or: return 1;
			case token_type::t_and: return 2;
			case token_type::t_pipe: return 3;
			case token_type::t_caret: return 4;
			case token_type::t_ampersand: return 5;
			case token_type::t_eq: case token_type::t_neq:
			case token_type::t_seq: case token_type::t_sneq: return 6;
			case token_type::t_lt: case token_type::t_gt:
			case token_type::t_leq: case token_type::t_geq: return 7;
			case token_type::t_lshift: case token_type::t_rshift: return 8;
			case token_type::t_plus: case token_type::t_minus: return 9;
			case token_type::t_star: case token_type::t_slash:
			case token_type::t_percent: return 10;
			default: return -1;
			}
		}
		...
		ast_ptr parse_expression(parser_state& s)
		{
			...
			if (s.check(token_type::t_question))
			{
				...
				auto ternary = make_node(node_type::n_ternary, "?:", expr->line, expr->column);
		...
		ast_ptr parse_block(parser_state& s)
		{
			auto block = make_node(node_type::n_block, "", s.current().line, s.current().column);
		...
		ast_ptr parse_if(parser_state& s)
		{
			...
			auto body = s.check(token_type::t_lbrace) ? parse_block(s) : parse_statement(s);
			...
			if (s.match(token_type::t_else))
			{
				...
				{
					auto else_body = s.check(token_type::t_lbrace) ? parse_block(s) : parse_statement(s);
		...
		ast_ptr parse_for(parser_state& s)
		{
			...
			node->children.push_back(init ? std::move(init) : make_node(node_type::n_undefined, "", tok.line, tok.column));
			node->children.push_back(cond ? std::move(cond) : make_node(node_type::n_true_val, "true", tok.line, tok.column));
			node->children.push_back(iter ? std::move(iter) : make_node(node_type::n_undefined, "", tok.line, tok.column));
			node->children.push_back(std::move(body));
			return node;
		}
		...
		ast_ptr parse_foreach(parser_state& s)
		{
			...
			if (s.match(token_type::t_comma))
			{
				...
				val_name = s.expect(token_type::t_identifier, "Expected value variable").value;
			}
			else
			{
			...
			// children[0] = key name (empty if single), children[1] = array expr, children[2] = body
			node->children.push_back(make_node(node_type::n_identifier, key_name, tok.line, tok.column));
		...
		ast_ptr parse_switch(parser_state& s)
		{
			...
			while (!s.check(token_type::t_rbrace) && !s.at_end())
			{
				if (s.check(token_type::t_case))
				{
					...
					auto case_node = make_node(node_type::n_case, "", case_tok.line, case_tok.column);
					case_node->children.push_back(std::move(case_val));

					auto body = make_node(node_type::n_block, "", case_tok.line, case_tok.column);
				...
				else if (s.check(token_type::t_default))
				{
					...
					auto def_node = make_node(node_type::n_default_case, "", def_tok.line, def_tok.column);
					auto body = make_node(node_type::n_block, "", def_tok.line, def_tok.column);
					while (!s.check(token_type::t_case) && !s.check(token_type::t_default)
						&& !s.check(token_type::t_rbrace) && !s.at_end())
					{
						body->children.push_back(parse_statement(s));
				...
				{
					throw std::runtime_error("Expected 'case' or 'default' in switch at line "
		...
		ast_ptr parse_statement(parser_state& s)
		{
			if (s.match(token_type::t_semicolon))
				return make_node(node_type::n_block, "", s.current().line, s.current().column);

			if (s.check(token_type::t_lbrace))
				return parse_block(s);

			if (s.check(token_type::t_if)) return parse_if(s);
			if (s.check(token_type::t_while)) return parse_while(s);
			if (s.check(token_type::t_do)) return parse_do_while(s);
			if (s.check(token_type::t_for)) return parse_for(s);
			if (s.check(token_type::t_foreach)) return parse_foreach(s);
			if (s.check(token_type::t_switch)) return parse_switch(s);
			...
			auto stmt = make_node(node_type::n_expression_stmt, "", expr->line, expr->column);
		...
		{
			auto params = make_node(node_type::n_block, "params", s.current().line, s.current().column);
			s.expect(token_type::t_lparen, "Expected '('");

			if (!s.check(token_type::t_rparen))
			{
				auto& p = s.expect(token_type::t_identifier, "Expected parameter name");
				params->children.push_back(make_node(node_type::n_identifier, p.value, p.line, p.column));

				while (s.match(token_type::t_comma))
				{
					auto& p2 = s.expect(token_type::t_identifier, "Expected parameter name");
					params->children.push_back(make_node(node_type::n_identifier, p2.value, p2.line, p2.column));
				}
			}
		...
		ast_ptr parse_function_def(parser_state& s, bool autoexec, bool is_private)
		{
			...
			auto func = make_node(node_type::n_function_def, name_tok.value, name_tok.line, name_tok.column);
			std::string flags_val;
			if (autoexec) flags_val = "autoexec";
			if (is_private)
			{
				if (!flags_val.empty()) flags_val += ",";
				flags_val += "private";
			}
			func->children.push_back(make_node(node_type::n_identifier, flags_val, name_tok.line, name_tok.column));
		...
		ast_ptr parse_script(parser_state& s)
		{
			...
			while (!s.at_end())
			{
				...
				{
					auto& tok = s.advance();
					auto& name = s.expect(token_type::t_identifier, "Expected namespace name");
					s.expect(token_type::t_semicolon, "Expected ';'");
					root->children.push_back(make_node(node_type::n_namespace, name.value, tok.line, tok.column));
					continue;
				}
				...
				if (s.check(token_type::t_using))
				{
					auto& tok = s.advance();
					std::string path;
					while (s.check(token_type::t_identifier) || s.check(token_type::t_backslash))
					{
						path += s.advance().value;
					}
					s.expect(token_type::t_semicolon, "Expected ';'");
					root->children.push_back(make_node(node_type::n_include, path, tok.line, tok.column));
					continue;
				}
				...
				if (s.check(token_type::t_precache))
				{
					...
					{
						if (s.check(token_type::t_lparen)) depth++;
						else if (s.check(token_type::t_rparen)) depth--;
				...
				while (s.check(token_type::t_function_kw) || s.check(token_type::t_autoexec) || s.check(token_type::t_private))
				{
					if (s.check(token_type::t_function_kw)) { s.advance(); saw_function_kw = true; }
					else if (s.check(token_type::t_autoexec)) { s.advance(); is_autoexec = true; }
					else if (s.check(token_type::t_private)) { s.advance(); is_private = true; }
				}
				...
				throw std::runtime_error("Unexpected token '" + s.current().value
					+ "' at top-level, line " + std::to_string(s.current().line));
			}
