...
namespace gsc_funcs
{
  namespace
  {
    ...
    void gscr_replacefunc(int inst)
    {
      ...
      {
        printf("^1[replacefunc] FAILED %s::%s -> %s::%s", target_script, target_func, replace_script, replace_func);
        if (!target_addr)
  ...
  struct component final : generic_component
  {
    void post_unpack() override
    {
      ...
      // Utility
      custom_builtins[static_cast<int32_t>(fnv1a("clearreplacefuncs"))] = gscr_clearreplacefuncs;
