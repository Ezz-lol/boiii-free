...
vm_opcode_handler_t orig_SafeCreateLocalVariables_gsc =
    nullptr; // table1 (inst 0)
vm_opcode_handler_t orig_SafeCreateLocalVariables_csc =
    nullptr; // table2 (inst 1)
...
void __fastcall hk_SafeCreateLocalVariables(int32_t inst, int64_t *fs_0,
  ...
  auto *orig = (inst == 0) ? orig_SafeCreateLocalVariables_gsc
                           : orig_SafeCreateLocalVariables_csc;
  if (orig)
    orig(inst, fs_0, vmc, terminate);
}
...
void __fastcall hk_CheckClearParams(int32_t inst, int64_t *fs_0, int64_t vmc,
  ...
  if (try_redirect(inst, fs_0))
    return; // VM re-dispatches from replacement's first opcode
  auto *orig =
      (inst == 0) ? orig_CheckClearParams_gsc : orig_CheckClearParams_csc;
  if (orig)
    orig(inst, fs_0, vmc, terminate);
}

void hook_opcode(size_t *table, uint16_t opcode, vm_opcode_handler_t hook,
  ...
  if (!hook)
    return; // just saving the original
...
struct component final : generic_component {
  void post_unpack() override {
    ...
    if (game::is_server()) {
      ...
      hook_opcode(table, 0x01D2, hk_SafeCreateLocalVariables,
                  &orig_SafeCreateLocalVariables_gsc);
      hook_opcode(table, 0x000D, hk_CheckClearParams,
                  &orig_CheckClearParams_gsc);
    } else {
      ...
      hook_opcode(table1, 0x01D2, hk_SafeCreateLocalVariables,
                  &orig_SafeCreateLocalVariables_gsc);
      hook_opcode(table1, 0x000D, hk_CheckClearParams,
                  &orig_CheckClearParams_gsc);
      hook_opcode(table2, 0x01D2, hk_SafeCreateLocalVariables,
                  &orig_SafeCreateLocalVariables_csc);
      hook_opcode(table2, 0x000D, hk_CheckClearParams,
                  &orig_CheckClearParams_csc);
    }
