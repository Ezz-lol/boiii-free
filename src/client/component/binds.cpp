...
namespace binds
{
	namespace
	{
		...
		void schedule_save()
		{
			if (save_scheduled.exchange(true)) return;
			scheduler::once([]
			{
		...
		void queue_op(pending_op op)
		{
			...
			scheduler::once([ver]
			{
				if (ver != pending_version.load()) return;
				flush_pending();
		...
		game::cmd_function_s* find_command(const char* name)
		{
			...
			{
				safety++;
				if (!IsBadReadPtr(cmd, sizeof(game::cmd_function_s))
					&& cmd->name
					&& !IsBadReadPtr(cmd->name, 1)
					&& _stricmp(cmd->name, name) == 0)
				{
					return cmd;
				}
				cmd = cmd->next;
		...
		void bind_wrapper()
		{
			if (original_bind_fn) original_bind_fn();

			if (current_phase != phase::ready) return;
		...
		void unbind_wrapper()
		{
			...
			if (original_unbind_fn) original_unbind_fn();
		...
		{
			if (original_unbindall_fn) original_unbindall_fn();
		...
		bool patch_command(const char* name, game::xcommand_t wrapper, game::xcommand_t* original_out)
		{
		...
		void parse_binds_file(const std::string& data)
		{
			...
			while (std::getline(stream, line))
			{
				...
				if (_strnicmp(line.c_str(), "bind ", 5) != 0) continue;
		...
		void init_hooks_and_load()
		{
			...
			if (!original_bind_fn)
			{
				printf("[Binds] FATAL: Could not hook 'bind' command — custom binds disabled\n");
