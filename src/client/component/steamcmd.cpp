...
namespace steamcmd
{
	namespace
	{
		...
		void monitor_initial_dump_phase(const std::string& workshop_id,
		                                std::chrono::steady_clock::time_point download_start)
		{
			const std::filesystem::path content_log_path = "./steamcmd/logs/content_log.txt";
	...
	void initialize_download(std::string workshop_id, std::string modtype)
	{
		if (workshop::launcher_downloading.load())
		{
			scheduler::once([]
			{
				game::UI_OpenErrorPopupWithMessage(0, game::ERROR_UI,
					"A download is already in progress from the launcher. Wait for it to finish.");
			}, scheduler::main);
			return;
		}
		...
		const std::string workshop_title = ws_info.title.empty()
		...
		const std::string content_folder = "./steamcmd/steamapps/workshop/content/311210/" + workshop_id;

		auto net_bytes_prev = std::make_shared<std::uint64_t>(0);
		auto net_baseline_set = std::make_shared<bool>(false);
		auto last_tick = std::make_shared<std::chrono::steady_clock::time_point>(std::chrono::steady_clock::now());
		auto download_start = std::make_shared<std::chrono::steady_clock::time_point>(std::chrono::steady_clock::now());
		auto warmup_phase = std::make_shared<bool>(true);
		auto download_phase_start = std::make_shared<std::chrono::steady_clock::time_point>();
		...
		scheduler::schedule([=]() -> bool
		{
			...
			{
				const auto wp_elapsed = std::chrono::steady_clock::now() - *download_phase_start;
				if (std::chrono::duration_cast<std::chrono::seconds>(wp_elapsed).count() >= 10)
					*warmup_phase = false;
			}

			if (*warmup_phase)
			{
				// Reset network baseline each warmup tick so the first real reading is clean
				...
				*last_tick = std::chrono::steady_clock::now();
				...
				s.active    = true;
				s.item_name = workshop_title;
				s.status_line = std::string("Preparing download... | Elapsed: ") + time_str;
			...
			*net_bytes_prev = net_bytes_now;
			...
			{
				const double raw_speed = (static_cast<double>(net_delta) * 1000.0) / static_cast<double>(dt_ms);
				...
				if (*smoothed_speed > 0.0)
					*speed_str = workshop::human_readable_size(static_cast<std::uint64_t>(*smoothed_speed)) + "/s";
			}
			*last_tick = now;
			...
			if (expected_size > 0 && display_size > 0 && speed > 1024.f)
			{
				...
				eta = static_cast<int>(static_cast<double>(remaining) / static_cast<double>(speed));
			}
		...
		if (result == 4)
			...
			error_msg = "There was a problem moving the workshop item to the correct folder.\nYou can try moving it manually and joining the server again.";
		else if (result == 1)
			error_msg = "Download cancelled.";

		if (error_msg)
		{
			scheduler::once([error_msg]
			{
	...
	int download_workshop_item(std::string workshop_id, std::string modtype)
	{
		...
		while (workshop::downloading_workshop_item && !std::filesystem::exists(content_folder))
		{
			...
			{
				printf("[ Workshop ] Too many quick failures (%d), resetting SteamCMD...\n", fast_fail_count);
				std::error_code ec;
			...
			// Detect rapid failures (SteamCMD exiting in under 15 seconds without producing content)
			auto attempt_elapsed = std::chrono::steady_clock::now() - attempt_start;
			if (std::chrono::duration_cast<std::chrono::seconds>(attempt_elapsed).count() < 15
