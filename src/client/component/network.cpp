...
int net_sendpacket_stub(const game::net::netsrc_t sock, const int length,
  ...
  buffer[0] = static_cast<char>((static_cast<uint32_t>(sock) & 0xF) |
                                ((to->localNetID & 0xF) << 4));
  memcpy(buffer.data() + 1, data, size);

  return sendto(s, buffer.data(), static_cast<int>(buffer.size()), 0,
...
struct component final : generic_component {
  void post_unpack() override {
    ...
    if (game::is_server()) {
      ...
      utils::hook::call(0x140538D5C_g,
                        con_restricted_execute_buf_stub); // SVC_RemoteCommand
