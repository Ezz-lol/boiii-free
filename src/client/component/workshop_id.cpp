...
int write_pubid_to_file(std::string pubID) {
  ...
  outputFile.close();
  std::string message =
      "Loaded workshop item id found! \n\nWriting ID to " + config_path +
      " as workshop_id dvar. \n\nPlease restart the server to enable it.";
  MessageBox(nullptr, message.c_str(), "Info!",
  ...
  if (utils::string::is_numeric(loaded_mod_id)) {
    ...
    for (const auto &entry :
      ...
      if (entry.is_directory() && entry.path().filename() == mapname.data()) {
        if (std::filesystem::exists(workshop_json) &&
            !std::filesystem::is_directory(workshop_json)) {
          read_json_for_id(workshop_json);
          break;
        }

        if (std::filesystem::exists(workshop_json_zone) &&
            !std::filesystem::is_directory(workshop_json_zone)) {
        ...
        } else if (std::filesystem::exists(workshop_json_zone) &&
                   !std::filesystem::is_directory(workshop_json_zone)) {
