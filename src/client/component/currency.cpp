...
/*
  CoD Points and Liquid Divinium are backed by the `bdMarketplace` Demonware
  service, which is a stub here - every task replies with an empty payload (see
  game/demonware/services/bdMarketplace.cpp). The engine's balance cache is
  therefore never populated, so both currencies read as zero and every purchase
  path bails out.

  Rather than emulate the marketplace wire protocol, this component keeps the
  wallet locally: it detours the LUI-facing accessors so the frontend reads our
  balance. It also stands in for the loot pipeline behind Dr. Monty's Factory,
  which is backed by the same dead service, and records what the machine hands
  out so it survives a restart.
*/
...
namespace currency {
namespace {
// Balances reach Lua through `lua_pushinteger`, which stores an HksNumber
// (a float). Keep the cap under 2^24 so every representable balance survives
// the round trip exactly - raising it past that starts losing units.
constexpr int64_t MAX_BALANCE = 9999999;

// A roll can only ever add one at a time, so this is a sanity bound on a
// hand-edited wallet rather than a rule the frontend enforces.
...
// Dr. Monty's Factory dispenses mega gobblegums, which are the
// `bubblegum_consumable` rows of `gamedata/stats/zm/zm_statsTable.csv`:
// column 0 is the item index, column 4 the reference the frontend wants back
// from `GetLootResults`. The table is baked in because the engine's own loot
// pool (`GetLootItems`) is served by Demonware and reads as empty here.
struct bgb_t {
  int32_t index;
...
// A pending machine run. `GetLootResults` is shared with the Black Market's
// decryption flow, so every stub below falls through to the engine unless we
// have a run of our own outstanding.
struct megachew_t {
  bool pending{false};
...
void save_locked(const wallet_t &w) {
  ...
  for (const auto &[mode, items] : w.owned) {
    ...
    for (const auto &[item_id, quantity] : items) {
      ...
      // Both name and value have to be rapidjson::Value: there is no
      // AddMember overload taking an rvalue name next to a plain integer.
...
// Keys are written as decimal strings; anything unparsable is skipped rather
// than throwing, so a hand-edited file cannot take the game down.
...
// The engine-facing side. Every detour below is a no-op unless the master
// switch is on, so a user can always get stock behaviour back.
...
// `SpendZMVials(controllerIndex, vats)` - one vat of Dr. Monty's Factory, one
// vial, one gobblegum. Deducting is only half of it: the frontend then polls
// the loot pipeline for what the machine produced, so queue that up here.
...
// `LootResultsReady(controllerIndex)` - the frontend polls this until the
// machine has something to show.
...
// `LootFailureReason(controllerIndex)` - without a live Demonware session the
// engine answers FAILURE here, which is what produced the factory's
// "An error has occurred running Dr. Monty's Machine" dialog.
...
// `GetLootResults(controllerIndex)` - a flat array of item references, one per
// vat powered. Anything else (item indices, or a padded fixed-width tuple)
// leaves the reveal unable to resolve the gobblegum and it renders blank.
...
// `GetLootItemQuantity(controllerIndex, reference, mode)` - this is what the
// gobblegum grid's count badge reads, by way of the frontend's
// `GetConsumableCountFromIndex`. Note the reference string: it is a different
// entry point from the engine's `Loot_GetItemQuantity(controller, mode, id)`
// that loot.cpp hooks, which is keyed by item index and never sees this call.
...
// Match tracking for the end-of-match award. Com_IsInGame is polled rather
// than hooking G_InitGame/G_ShutdownGame because those only run on the machine
// hosting the match - polling also covers playing on someone else's dedi.
...
void award_for_match(const std::chrono::seconds duration) {
  ...
  if (!summary.empty()) {
    toast::success("Match Rewards", summary);
  }
}
...
// Console commands. `codpoints`/`divinium` with no argument report the
// balance, with an argument set it.
...
struct component final : client_component {
  void post_unpack() override {
    ...
    // Loaded eagerly rather than on `dvars_loaded`: the wallet does not
    // depend on user config, and the frontend can query a balance before that
    // pipeline runs.
    load();
