...
namespace toast
{
	...
	void show(const std::string& title, const std::string& description, const std::string& icon)
	{
		if (game::is_server()) return;
		if (game::Com_IsInGame()) return;
		...
		const std::string code = utils::string::va(
			"pcall(function()\n"
			"  local ctrl = Engine.GetModelForController(Engine.GetPrimaryController())\n"
			"  local toast = Engine.CreateModel(ctrl, \"FrontendToast\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"state\"), \"DefaultState\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"kicker\"), \"%s\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"description\"), \"%s\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"contentIcon\"), \"%s\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"functionIcon\"), \"blacktransparent\")\n"
			"  Engine.SetModelValue(Engine.CreateModel(toast, \"backgroundId\"), \"blacktransparent\")\n"
