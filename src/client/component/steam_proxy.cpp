...
bool perform_cleanup_if_needed() {
  ...
  if (steam_client_module && steam_pipe && global_user &&
      steam_client_module.invoke<bool>("Steam_BConnected", global_user,
                                       steam_pipe) &&
      steam_client_module.invoke<bool>("Steam_BLoggedOn", global_user,
                                       steam_pipe)) {
    return false;
  }
...
ownership_state start_mod_unsafe(const std::string &title, size_t app_id) {
  ...
  const auto _0 = utils::finally([] { set_pipe_app_id(workshop_app_id); });
  ...
  client_user.invoke<bool>("SpawnProcess", path.generic_string().data(),
                           cmdline, our_directory, &game_id.bits, title.data(),
                           0, 0, 0);
...
void evaluate_ownership_state(const ownership_state state) {
  ...
  // throw std::runtime_error("You must own the game on steam to play this
  // mod!");
...
struct component final : client_component {
  ...
  void pre_destroy() override {
    if (steam_client) {
      ...
      (void)steam_client_module.invoke<bool>("Steam_BReleaseSteamPipe",
                                             steam_pipe);
    }
  }
...
const utils::nt::library &get_overlay_module() { return steam_overlay_module; }

bool set_rich_presence(const std::string &key, const std::string &value) {
  if (steam_friends_real) {
    try {
      return steam_friends_real.invoke<bool>(43, key.c_str(), value.c_str());
    } catch (...) {
    }
  }
  if (client_friends) {
    try {
      return client_friends.invoke<bool>("SetRichPresence", launch_app_id,
...
void clear_rich_presence() {
  if (steam_friends_real) {
    try {
      steam_friends_real.invoke<void>(44);
      return;
    } catch (...) {
    }
  }
  if (client_friends) {
    try {
      client_friends.invoke<void>("ClearRichPresence", launch_app_id);
    } catch (...) {
    }
  }
}
...
void initialize() {
  ...
  steam_client = steam_client_module.invoke<steam::client *>(
      "CreateInterface", "SteamClient017", nullptr);
  if (!steam_client)
    return;
...
void update_map_client(subscribed_item_map &map) {
  ...
  for (uint32_t i = 0; i < result; ++i) {
    ...
    item.available = client_ugc.invoke<bool>("GetItemInstallInfo", app_id,
                                             ids[i], &item.size_on_disk, buffer,
...
void update_map_steam(subscribed_item_map &map) {
  ...
  for (uint32_t i = 0; i < result; ++i) {
    ...
    item.available = steam_ugc->GetItemInstallInfo(
        ids[i], &item.size_on_disk, buffer, sizeof(buffer), &item.time_stamp);
    item.path = buffer;
...
void update_subscribed_items() {
  ...
  const auto _ = utils::finally([&] {
    subscribed_items.access(
        [&](subscribed_item_map &items) { items = std::move(map); });
  });
...
void access_steam_friends(
    const std::function<void(
        const std::vector<std::pair<uint64_t, std::string>> &)> &callback) {
