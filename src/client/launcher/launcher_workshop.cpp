...
{"id":"3649285026","title":"HOGZMEADE ZOMBIES","imageUrl":"https://images.steamusercontent.com/ugc/9770236674819273014/98341B495C30B4D9B4A779C34939019E8A0D333A/?imw=200&imh=200&ima=fit&impolicy=Letterbox&imcolor=%23000000&letterbox=true","description":"The All Wizarding Village of Hogsmeade. 4 PERK LIMIT.","file_size":13099650253,"starRating":4,"subs":5501,"favorites":289}
    ...
    void kill_process_tree(DWORD parent_pid)
    {
        ...
        HANDLE proc = OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, parent_pid);
        if (proc) {
    ...
    void force_delete_folder(const std::filesystem::path& dir)
    {
        ...
        for (int attempt = 0; attempt < 5; ++attempt) {
            ...
            for (const auto& entry : std::filesystem::recursive_directory_iterator(
                ...
                if (ec) break;
                try {
                    SetFileAttributesW(entry.path().wstring().c_str(), FILE_ATTRIBUTE_NORMAL);
                } catch (...) {}
            }
            ...
            for (const auto& entry : std::filesystem::recursive_directory_iterator(
                ...
                if (ec) break;
                if (entry.is_regular_file(ec))
                    files_to_del.push_back(entry.path());
                else if (entry.is_directory(ec))
                    dirs_to_del.push_back(entry.path());
            }
            for (const auto& f : files_to_del) {
                ec.clear();
                SetFileAttributesW(f.wstring().c_str(), FILE_ATTRIBUTE_NORMAL);
                std::filesystem::remove(f, ec);
            }
            std::sort(dirs_to_del.begin(), dirs_to_del.end(),
                [](const std::filesystem::path& a, const std::filesystem::path& b) {
                    return a.wstring().size() > b.wstring().size();
                });
            for (const auto& d : dirs_to_del) {
                ec.clear();
        ...
        try {
            ...
            if (!std::filesystem::exists(steamcmd_exe)) {
                ...
                const auto zip_data = utils::http::get_data(
                    "https://steamcdn-a.akamaihd.net/client/installer/steamcmd.zip", {}, {}, 3);
                if (!zip_data || zip_data->empty()) {
                    set_workshop_status("Error: Failed to download SteamCMD.", 0.0,
                        "Could not reach steamcdn-a.akamaihd.net. Check your internet connection.");
                    return;
                }
...

            {
                ...
                if (!ec && exe_size < 3 * 1024 * 1024) {
                    ...
                    if (CreateProcessA(nullptr, const_cast<char*>(init_cmd.c_str()), nullptr, nullptr,
                            FALSE, 0, nullptr, steamcmd_dir_str.c_str(), &si, &pi)) {
                    ...
                    if (!ec && new_size < 3 * 1024 * 1024) {
                        set_workshop_status("Error: SteamCMD failed to initialize.", 0.0,
                            "The self-update may have failed. Try again or check your internet connection.");
                        return;
                    }
                }
            }
            ...
            const std::string expected_size_str = expected_size > 0 ? human_readable_size(expected_size) : "";
                    ...
                    if (free_bytes_available.QuadPart < required_space) {
                        set_workshop_status(
                            ...
                            "Need ~" + human_readable_size(required_space)
                                + " free, but only " + human_readable_size(free_bytes_available.QuadPart)
            ...
            auto clean_all_workshop_folders = [&]() {
                ...
                force_delete_folder(alt_content_path);
                force_delete_folder(alt_download_path);
                force_delete_folder(steamcmd_dir / "steamapps" / "workshop" / "downloads" / "311210");
                force_delete_folder(game_path / "steamapps" / "workshop" / "downloads" / "311210");
            };
            ...
            while (!workshop_cancel_requested.load()) {
                internal_retry++;
                if (internal_retry > MAX_RETRIES) {
                    set_workshop_status("Error: Download failed.", 0.0,
                        "SteamCMD could not complete the download after multiple attempts.\n"
                        "Try again or check your connection.");
                    return;
                }

                if (visible_retry > 0) {
                    set_workshop_status("Restarting download: " + workshop_title, -1.0,
                        expected_size_str.empty() ? ("Workshop ID: " + workshop_id) : ("File size: " + expected_size_str));
                } else {
                    set_workshop_status("Downloading " + workshop_title + "...",
                        expected_size > 0 ? 0.1 : -1.0,
                        expected_size_str.empty() ? ("Workshop ID: " + workshop_id) : ("File size: " + expected_size_str));
                }
                ...
                std::uint64_t prev_folder_size = 0;
                ...
                for (;;) {
                    ...
                    {
                        std::error_code fec;
                        if (std::filesystem::exists(content_path, fec)) {
                        ...
                        if (std::filesystem::exists(download_path, fec)) {
                            ...
                            if (active_folder.empty()) active_folder = download_path.string();
                        }
                        if (std::filesystem::exists(alt_content_path, fec)) {
                            const auto alt_sz = compute_folder_size_bytes(alt_content_path);
                            if (alt_sz > current_size) {
                                current_size = alt_sz;
                                active_folder = alt_content_path.string();
                            }
                        }
                        if (std::filesystem::exists(alt_download_path, fec)) {
                            current_size += compute_folder_size_bytes(alt_download_path);
                            if (active_folder.empty()) active_folder = alt_download_path.string();
                        }
                    }

                    {
                        ...
                        if (dt_ms > 0 && current_size > prev_folder_size) {
                            const std::uint64_t size_delta = current_size - prev_folder_size;
                            const double bytes_per_sec = (static_cast<double>(size_delta) * 1000.0) / static_cast<double>(dt_ms);
                            if (bytes_per_sec > 0.0) {
                                last_speed_str = human_readable_size(static_cast<std::uint64_t>(bytes_per_sec)) + "/s";
                            }
                        }
                        last_tick = now;
                        prev_folder_size = current_size;
                    }
                    ...
                    if (expected_size > 0 && current_size > 0) {
                        percent = (static_cast<double>(current_size) / static_cast<double>(expected_size)) * 100.0;
                    ...
                    std::string details;
                    {
                        ...
                        if (current_size > 0 && expected_size > 0) {
                            details = human_readable_size(current_size) + " / " + expected_size_str;
                        } else if (current_size > 0) {
                        ...
                        if (visible_retry > 0) {
                            details += " | Retry " + std::to_string(visible_retry);
                        }
                    }
                ...
                {
                    std::error_code ec;
                    auto final_size = compute_folder_size_bytes(content_path);
                    if (final_size > 1024) {
                        break;
                    }
                    if (std::filesystem::exists(alt_content_path, ec)) {
                        const auto alt_size = compute_folder_size_bytes(alt_content_path);
                        if (alt_size > 1024) {
