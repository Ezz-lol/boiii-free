...
void set_remove_status(const std::string &msg, double pct,
...
void verify_game_thread(const std::string &modes_csv) {
  ...
  try {
    ...
    if (!std::filesystem::exists(manifest_path)) {
      set_verify_status(
          "verification.json not found in " + manifest_path.string(), 0.0, "");
    ...
    auto should_include_component = [&](const std::string &comp) -> bool {
      if (comp == "redist")
        return false; // skip redistributables
      if (comp == "base")
        return true;
      if (comp == "sp")
        return want_cp;
      if (comp == "zc")
        return want_zm;
      if (comp == "dlc")
        return want_mp || want_zm;
      return want_all;
    };

    std::vector<std::string> exclude_prefixes;
    if (!want_cp) {
      exclude_prefixes.push_back("cp_");
      exclude_prefixes.push_back("en_cp_");
    }
    if (!want_mp) {
      exclude_prefixes.push_back("mp_");
      exclude_prefixes.push_back("en_mp_");
    }
    if (!want_zm) {
      exclude_prefixes.push_back("zm_");
      exclude_prefixes.push_back("en_zm_");
    }
    ...
    for (rapidjson::SizeType i = 0; i < files_array.Size(); i++) {
      ...
      std::uint64_t size = 0;
      if (entry[1].IsUint64())
        size = entry[1].GetUint64();
      else if (entry[1].IsInt64())
        size = static_cast<std::uint64_t>(entry[1].GetInt64());
      else if (entry[1].IsUint())
        size = entry[1].GetUint();
      std::string hash_str;
      if (entry.Size() >= 3 && entry[2].IsString())
        hash_str = entry[2].GetString();
      std::string comp = entry[3].GetString();

      if (!should_include_component(comp))
        continue;
    ...
    const std::uint32_t error_total =
        missing_count + size_mismatch_count + hash_mismatch_count;
  ...
  for (const auto &dir : dirs_to_scan) {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(dir, ec)) {
      ...
      if (!has_image_extension(entry.path()))
        continue;
      std::string abs_path =
          std::filesystem::absolute(entry.path(), ec).generic_string();
      if (path_starts_with_ignore_case(stem_lower, "previewimage")) {
        if (found_preview.empty())
          found_preview = abs_path;
      } else if (path_starts_with_ignore_case(stem_lower, "loadingimage")) {
        if (found_loading.empty())
          found_loading = abs_path;
...
bool try_parse_workshop_json(const std::filesystem::path &dir,
                             mod_item_info &item) {
  std::filesystem::path zone_json = dir / "zone" / "workshop.json";
  if (!utils::io::file_exists(zone_json.string()))
    return false;
  std::string data;
  if (!utils::io::read_file(zone_json.string(), &data))
    return false;
  rapidjson::Document doc;
  ...
  if (doc.HasMember("PublisherID")) {
    ...
    if (pid.IsString())
      ...
      item.id = std::to_string(pid.GetInt64());
    else if (pid.IsUint64())
      item.id = std::to_string(pid.GetUint64());
  }
  return true;
}
...
std::string workshop_list_json() {
  ...
  for (const auto &it : items) {
    ...
    w.Key("folder");
    w.String(it.folder.c_str());
    w.Key("type");
    w.String(it.type.c_str());
    w.Key("id");
    w.String(it.id.c_str());
    if (!it.image.empty()) {
      w.Key("image");
      w.String(it.image.c_str());
    }
    if (!it.source.empty()) {
      w.Key("source");
      w.String(it.source.c_str());
    }
    {
      w.Key("path");
      if (!it.path.empty())
        w.String(it.path.c_str());
      else
        w.String(it.dir_path.string().c_str());
    }

    w.Key("localSize");
    w.Uint64(it.local_size);
    if (!it.description.empty()) {
      w.Key("description");
      w.String(it.description.c_str());
    }
    ...
    if (!it.id.empty()) {
      ...
      if (mi != meta_map.end()) {
        ...
        if (!m.description.empty() && it.description.empty()) {
          w.Key("description");
          w.String(m.description.c_str());
        }
        if (it.image.empty() && !m.preview_url.empty()) {
          w.Key("image");
          w.String(m.preview_url.c_str());
        }
      }
    }
    w.EndObject();
