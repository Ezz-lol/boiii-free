...
bool byte_buffer::read_string(char *output, const int length) {
  ...
  utils::string::copy(output, static_cast<size_t>(length),
                      this->buffer_.data() + this->current_byte_);
  this->current_byte_ += strlen(output) + 1;
...
bool byte_buffer::read_array_header(const unsigned char expected,
  ...
  if (element_size)
    *element_size = array_size / el_count;
...
size_t byte_buffer::size() const { return this->buffer_.size(); }

bool byte_buffer::is_using_data_types() const { return use_data_types_; }

std::string &byte_buffer::get_buffer() { return this->buffer_; }

std::string byte_buffer::get_remaining() {
  return std::string(this->buffer_.begin() + this->current_byte_,
                     this->buffer_.end());
}
