...
namespace demonware {
template <typename T> class server_registry {
  static_assert(std::is_base_of_v<base_server, T>,
                "Invalid server registry type");

public:
  template <typename S, typename... Args> void create(Args &&...args) {
  ...
  T *find(const std::string &name) {
    const auto address =
        utils::cryptography::jenkins_one_at_a_time::compute(name);
    return find(address);
  }
