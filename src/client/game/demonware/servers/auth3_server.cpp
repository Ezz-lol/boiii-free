...
void auth3_server::handle(const std::string &packet) {
  ...
  std::string session_key("\x13\x37\x13\x37\x13\x37\x13\x37\x13\x37\x13\x37\x13"
                          "\x37\x13\x37\x13\x37\x13\x37\x13\x37\x13\x37",
                          24);
  ...
  const auto iv = utils::cryptography::tiger::compute(
      std::string(reinterpret_cast<char *>(&iv_seed), 4));
  const auto ticket_enc = utils::cryptography::des3::encrypt(
      std::string(reinterpret_cast<char *>(&ticket), sizeof(ticket)), iv,
      auth_key);
  ...
  std::string username =
      std::string(ticket.m_username, sizeof(ticket.m_username)).data();
  ...
  result.append(
      utils::string::va("Content-Length: %d\r\n\r\n", buffer.GetLength()));
