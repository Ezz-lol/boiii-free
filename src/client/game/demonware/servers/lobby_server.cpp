    ...
    while (buffer.has_more_data()) {
      ...
      if (size == 0xC8) {
        ...
        const std::string packet_2(
            "\x16\x00\x00\x00\xab\x81\xd2\x00\x00\x00\x13\x37\x13\x37\x13\x37"
            "\x13\x37\x13\x37\x13\x37\x13\x37\x13\x37",
            26);
...
#endif
          std::string packet_3(packet.data(),
                               packet.size() -
                                   8); // this 8 are client hash check?
