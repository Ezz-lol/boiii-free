...
std::string encrypted_reply::data() {
  ...
  enc_buffer.write_uint32(static_cast<unsigned int>(
      this->buffer_.size()));           // service data size CHECKTHIS!!
  ...
  // seed
  std::string seed(
      "\x5E\xED\x5E\xED\x5E\xED\x5E\xED\x5E\xED\x5E\xED\x5E\xED\x5E\xED", 16);

  // encrypt
  const auto enc_data =
      utils::cryptography::aes::encrypt(aligned_data, seed, get_encrypt_key());
  ...
  response.write_ubyte(0xAB);
  response.write_ubyte(0x85);
  response.write_int32(msg_count);
  response.write(16, seed.data());
  response.write(enc_data);

  // hash entire packet and append end
  auto hash_data = utils::cryptography::hmac_sha1::compute(
      response.get_buffer(), get_hmac_key());
  hash_data.resize(8);
