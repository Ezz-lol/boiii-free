...
namespace demonware
{
	...
	std::string encrypted_reply::data()
	{
		...
		// encrypt
		const auto enc_data = utils::cryptography::aes::encrypt(aligned_data, seed, get_encrypt_key());
		...
		auto hash_data = utils::cryptography::hmac_sha1::compute(response.get_buffer(), get_hmac_key());
		hash_data.resize(8);
