...
public:
  ...
  std::string data() override { return this->buffer_; }
};
...
public:
  ...
  encrypted_reply(const uint8_t type, byte_buffer *bbuffer)
      : typed_reply(type) {
    this->buffer_.append(bbuffer->get_buffer());
  }
...
public:
  unencrypted_reply(const uint8_t _type, bit_buffer *bbuffer)
      : typed_reply(_type) {
    this->buffer_.append(bbuffer->get_buffer());
  }

  unencrypted_reply(const uint8_t _type, byte_buffer *bbuffer)
      : typed_reply(_type) {
    this->buffer_.append(bbuffer->get_buffer());
  }
...
public:
  remote_reply(service_server *server, uint8_t _type)
      : type_(_type), server_(server) {}
