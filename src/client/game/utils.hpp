...
namespace game
{
	...
	int set_dvar_int(const char* dvar_name, int val, DvarSetSource source = DVAR_SOURCE_INTERNAL);
	int64_t set_dvar_int64(const char* dvar_name, int64_t val, DvarSetSource source = DVAR_SOURCE_INTERNAL);
	uint64_t set_dvar_uint64(const char* dvar_name, uint64_t val, DvarSetSource source = DVAR_SOURCE_INTERNAL);
	bool set_dvar_bool(const char* dvar_name, bool val, DvarSetSource source = DVAR_SOURCE_INTERNAL);
	float set_dvar_float(const char* dvar_name, float val, DvarSetSource source = DVAR_SOURCE_INTERNAL);


	[[nodiscard]] const dvar_t* register_dvar_bool(const char* dvar_name, bool value, unsigned int flags,
	                                               const char* description);
	[[nodiscard]] const dvar_t* register_dvar_int(const char* dvar_name, int value, int min, int max,
	                                              unsigned int flags, const char* description);
	[[nodiscard]] const dvar_t* register_dvar_float(const char* dvar_name, float value, float min, float max,
	                                                unsigned int flags, const char* description);
	[[nodiscard]] const dvar_t* register_sessionmode_dvar_bool(const char* dvar_name, bool value, unsigned int flags,
	                                                           const char* description, eModes mode = MODE_COUNT);
	[[nodiscard]] const dvar_t* register_dvar_string(const char* dvar_name, const char* value, unsigned int flags,
	                                                 const char* description);
