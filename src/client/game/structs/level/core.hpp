...
enum class entityType_t : uint32_t {
  ...
  */
  ...
  /*
   Entity events are tagged with an entityType >= 0x18.
   They are accessed with `entityType - 24` and handled in several
   CG_ functions in the engine which match and handle each entity event type.
  */
